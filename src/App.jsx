  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      [[0.2, 0.3, "#00E676", 0.04], [0.8, 0.7, "#00BCD4", 0.03], [0.5, 0.9, "#69F0AE", 0.025]].forEach(([rx, ry, c, a]) => {
        const gx = rx * canvas.width + Math.sin(f * 0.005) * 60;
        const gy = ry * canvas.height + Math.cos(f * 0.004) * 40;
      ...
      state.current.quotes.forEach(q => {
        q.x += q.vx; q.y += q.vy;
        if (q.x < -300) q.x = canvas.width + 50;
        if (q.x > canvas.width + 300) q.x = -50;
        if (q.y < -30) q.y = canvas.height + 10;
        if (q.y > canvas.height + 30) q.y = -10;
        ctx.save();
        ctx.globalAlpha = q.opacity;
        ctx.font = `900 ${q.size}px 'Segoe UI', sans-serif`;
      ...
      state.current.particles.forEach(p => {
        ...
        ctx.fillStyle = `rgba(0,230,118,${p.opacity + Math.sin(p.pulse) * 0.05})`;
        ctx.fill();
      });
      ...
      scanG.addColorStop(0.5, "rgba(0,230,118,0.025)");
    ...
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 2, overflow: "hidden", background: "linear-gradient(90deg,#0a0a0f,#0a0a0fee)", borderTop: "1px solid #00E67622", padding: "7px 0" }}>
      <div style={{ display: "flex", animation: "ticker 40s linear infinite", whiteSpace: "nowrap", width: "max-content" }}>
        {[...QUOTES, ...QUOTES].map((q, i) => (
          <span key={i} style={{ marginRight: 60, color: i % 3 === 0 ? "#00E676" : i % 3 === 1 ? "#69F0AE" : "#00BCD4", fontSize: 11, fontWeight: 900, letterSpacing: 2, textTransform: "uppercase", opacity: 0.7 }}>
            ⚡ {q}
          </span>
        ))}
      </div>
...
const Textarea = ({ label, value, onChange, placeholder, hint }) => (
  <div style={{ marginBottom: 16 }}>
    ...
    {hint && <div style={{ color: "#445", fontSize: 12, marginBottom: 6 }}>{hint}</div>}
    <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={3}
      style={{ width: "100%", background: "rgba(0,0,0,0.5)", border: "1.5px solid #1e2a1e", borderRadius: 10, padding: "12px 16px", color: "#fff", fontSize: 14, outline: "none", boxSizing: "border-box", resize: "vertical", fontFamily: "inherit", backdropFilter: "blur(4px)" }}
...
const Select = ({ label, value, onChange, options, required }) => (
  <div style={{ marginBottom: 16 }}>
    ...
    <select value={value} onChange={e => onChange(e.target.value)}
      style={{ width: "100%", background: "rgba(0,0,0,0.6)", border: "1.5px solid #1e2a1e", borderRadius: 10, padding: "12px 16px", color: value ? "#fff" : "#556", fontSize: 15, outline: "none", boxSizing: "border-box" }}>
      ...
      {options.map(o => {
        ...
        return (
          <button key={o} onClick={() => onChange(active ? selected.filter(x => x !== o) : [...selected, o])}
            style={{ padding: "7px 14px", borderRadius: 20, border: active ? "1.5px solid #00E676" : "1.5px solid #1e3020", background: active ? "linear-gradient(135deg,#00E67628,#00BCD428)" : "rgba(0,0,0,0.4)", color: active ? "#00E676" : "#667", fontSize: 12, fontWeight: 800, cursor: "pointer", transition: "all 0.2s", boxShadow: active ? "0 0 10px #00E67633" : "none" }}>
            {o}
          </button>
        );
      })}
...
const OptionCard = ({ label, desc, selected, onClick }) => (
  <div onClick={onClick}
    ...
    {selected && <div style={{ color: "#00E676", fontSize: 18, fontWeight: 900 }}>✓</div>}
  </div>
);
...
const BMRInfo = ({ form }) => {
  ...
  const hCm = form.heightUnit === "in" ? h * 2.54 : h;
      ...
      {steps.map((s, i) => (
        <div key={s.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1 }}>
          <div style={{
            ...
            background: i < current ? "#00E676" : i === current ? "linear-gradient(135deg,#00E676,#00BCD4)" : "rgba(0,0,0,0.4)",
            border: i === current ? "2px solid #00E676" : i < current ? "2px solid #00E676" : "1.5px solid #1e3020",
            color: i <= current ? "#0a0a0f" : "#334", fontWeight: 900, fontSize: 10,
            boxShadow: i === current ? "0 0 14px #00E67677" : "none", transition: "all 0.3s"
          }}>{i < current ? "✓" : i+1}</div>
...
// ── Step accent colors cycling ─────────────────────────────────────────
const stepAccents = [
  { from: "#00E676", to: "#00BCD4" },
  { from: "#69F0AE", to: "#00E676" },
  { from: "#00BCD4", to: "#26C6DA" },
  { from: "#00E676", to: "#69F0AE" },
  { from: "#1DE9B6", to: "#00E676" },
  { from: "#00BCD4", to: "#00E676" },
  { from: "#69F0AE", to: "#1DE9B6" },
  { from: "#00E676", to: "#00BCD4" },
];
...
function PlanView({ form, plan, onRestart, onRegenerate }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", padding: "24px 16px 60px", fontFamily: "'Segoe UI',sans-serif", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", position: "relative", zIndex: 2 }}>
        ...
        <div style={{ background: "rgba(10,12,10,0.85)", border: "1px solid #1a2a1a", borderRadius: 16, padding: "24px 20px", lineHeight: 1.8, color: "#bbb", fontSize: 14, whiteSpace: "pre-wrap", backdropFilter: "blur(12px)", marginBottom: 14, boxShadow: "0 8px 32px #00000099" }}>
          {currentPlan.split('\n').map((line, i) => {
            const isDay = /^DAY\s*\d/i.test(line.trim());
            const isMeal = /^(BREAKFAST|LUNCH|DINNER|SNACK|MORNING SNACK|EVENING SNACK|PRE-WORKOUT|POST-WORKOUT)/i.test(line.trim());
            return (
              <div key={i} style={{
                color: isDay ? "#0a0a0f" : isMeal ? "#00E676" : "#bbb",
                fontWeight: isDay || isMeal ? 900 : 400,
                fontSize: isDay ? 11 : 14,
                background: isDay ? "linear-gradient(90deg,#00E676,#00BCD4)" : "transparent",
                padding: isDay ? "5px 14px" : 0,
                borderRadius: isDay ? 6 : 0,
                display: isDay ? "inline-block" : "block",
                marginTop: isDay ? 18 : isMeal ? 10 : 0,
                marginBottom: isDay ? 10 : 0,
                textTransform: isDay || isMeal ? "uppercase" : "none",
                letterSpacing: isDay || isMeal ? 1.5 : 0,
                textShadow: isMeal ? "0 0 12px #00E67644" : "none",
              }}>{line.replace(/[#*]/g, "").trim()}</div>
            );
          })}
        </div>
        <button onClick={() => setShowChat(s => !s)}
          style={{ width: "100%", padding: 14, background: showChat ? "rgba(0,230,118,0.08)" : "linear-gradient(135deg,rgba(0,230,118,0.1),rgba(0,188,212,0.08))", border: "1.5px solid #00E676", borderRadius: 12, color: "#00E676", fontWeight: 900, fontSize: 14, cursor: "pointer", textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 12, boxShadow: "0 0 16px #00E67622" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {["Remove all tofu","Replace chicken with fish","I want more protein","Make Day 3 vegetarian","Reduce carbs on Day 5"].map(s => (
                    <button key={s} onClick={() => setChatInput(s)}
                      style={{ padding: "6px 12px", borderRadius: 14, border: "1px solid #1e2a1e", background: "rgba(0,0,0,0.3)", color: "#556", fontSize: 11, cursor: "pointer", fontWeight: 700 }}>
                      {s}
                    </button>
                  ))}
                </div>
    ...
    switch(step) {
      case 0: return <>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>LET'S GET TO<br /><span style={{ background: `linear-gradient(90deg,${accent.from},${accent.to})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>KNOW YOU</span></div>
          <div style={{ color: "#446644", marginTop: 8, fontSize: 13 }}>Your journey to a better you starts here 🔥</div>
        </div>
      ...
      case 1: return <>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>YOUR BODY<br /><span style={{ background: `linear-gradient(90deg,${accent.from},${accent.to})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>STATS</span></div>
          <div style={{ color: "#446644", marginTop: 8, fontSize: 13 }}>We'll calculate your perfect calories 📊</div>
        </div>
        <div style={{ display:"flex", gap:12 }}>
      ...
      case 2: return <>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>WHAT'S YOUR<br /><span style={{ background: `linear-gradient(90deg,${accent.from},${accent.to})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>GOAL?</span></div>
          <div style={{ color: "#446644", marginTop: 8, fontSize: 13 }}>Define your target and we'll build around it 🎯</div>
        </div>
        {[{v:"lose-weight",label:"🔥 Lose Weight",desc:"Burn fat, get lean"},{v:"gain-muscle",label:"💪 Build Muscle",desc:"Increase strength & size"},{v:"maintain",label:"⚖️ Maintain Weight",desc:"Stay at current weight"},{v:"improve-health",label:"🌿 Improve Health",desc:"Boost energy & wellness"},{v:"bulk",label:"📈 Bulk Up",desc:"Serious mass gain"}].map(g=><OptionCard key={g.v} label={g.label} desc={g.desc} selected={form.goal===g.v} onClick={()=>upd("goal",g.v)} />)}
      </>;
      case 3: return <>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>YOUR<br /><span style={{ background: `linear-gradient(90deg,${accent.from},${accent.to})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>LIFESTYLE</span></div>
          <div style={{ color: "#446644", marginTop: 8, fontSize: 13 }}>How active is your daily life? 🏃</div>
        </div>
        {[{v:"sedentary",label:"🛋️ Sedentary",desc:"Desk job, little exercise"},{v:"light",label:"🚶 Lightly Active",desc:"Light exercise 1-3 days/week"},{v:"moderate",label:"🏋️ Moderately Active",desc:"Exercise 3-5 days/week"},{v:"active",label:"⚡ Very Active",desc:"Hard exercise 6-7 days/week"},{v:"very-active",label:"🔥 Athlete",desc:"Physical job + daily training"}].map(a=><OptionCard key={a.v} label={a.label} desc={a.desc} selected={form.activityLevel===a.v} onClick={()=>upd("activityLevel",a.v)} />)}
        <BMRInfo form={form} />
      ...
      case 4: return <>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>DIET<br /><span style={{ background: `linear-gradient(90deg,${accent.from},${accent.to})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>PREFERENCES</span></div>
          <div style={{ color: "#446644", marginTop: 8, fontSize: 13 }}>Tell us what works for you 🥦</div>
        </div>
      ...
      case 5: return <>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>FOOD<br /><span style={{ background: `linear-gradient(90deg,${accent.from},${accent.to})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>ACCESS</span></div>
          <div style={{ color: "#446644", marginTop: 8, fontSize: 13 }}>Build your plan around what you actually have 🛒</div>
        </div>
      ...
      case 6: return <>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>SUPPLEMENTS<br /><span style={{ background: `linear-gradient(90deg,${accent.from},${accent.to})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>& BOOSTERS</span></div>
          <div style={{ color: "#446644", marginTop: 8, fontSize: 13 }}>Do you take or plan to take supplements? 💊</div>
        </div>
        {[{v:"yes",label:"💊 Yes, I use supplements",desc:"Include them in my plan"},{v:"no",label:"🙅 No supplements",desc:"Keep it food-only"},{v:"open",label:"🤔 Open to suggestions",desc:"Recommend what fits my goal"}].map(o=><OptionCard key={o.v} label={o.label} desc={o.desc} selected={form.useSupplements===o.v} onClick={()=>upd("useSupplements",o.v)} />)}
      ...
      case 7: return <>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>ALMOST<br /><span style={{ background: `linear-gradient(90deg,${accent.from},${accent.to})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>THERE!</span></div>
          <div style={{ color: "#446644", marginTop: 8, fontSize: 13 }}>Last few details and we're set 💯</div>
        </div>
        ...
        <div style={{ background: "linear-gradient(135deg,rgba(0,20,10,0.7),rgba(0,10,20,0.7))", border: "1px solid #00E67622", borderRadius: 12, padding: 16, marginTop: 8, backdropFilter: "blur(8px)" }}>
          ...
          {[["Name",form.name],["Goal",form.goal?.replace(/-/g," ").toUpperCase()],["Activity",form.activityLevel?.replace(/-/g," ").toUpperCase()],["Diet",form.dietType.join(", ")||"Standard"],["Meals/day",form.mealsPerDay],["Supplements",form.useSupplements==="no"?"None":[...form.supplements,form.customSupplement].filter(Boolean).join(", ")||"Open"]].map(([k,v])=>v?(
            <div key={k} style={{ display:"flex", justifyContent:"space-between", borderBottom:"1px solid #0a140a", padding:"5px 0", fontSize:12 }}>
              <span style={{ color:"#334" }}>{k}</span><span style={{ color:"#ccc", fontWeight:700 }}>{v}</span>
            </div>
          ):null)}
        </div>
      </>;
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0a0a0f", fontFamily:"'Segoe UI',sans-serif", padding:"24px 16px 80px", position:"relative" }}>
      ...
      <div style={{ maxWidth:480, margin:"0 auto", position:"relative", zIndex:2 }}>
        ...
        {/* Glowing card */}
        <div style={{
          background: "linear-gradient(160deg,rgba(0,15,8,0.92) 0%,rgba(0,8,15,0.92) 100%)",
          border: `1.5px solid`,
          borderImage: `linear-gradient(135deg,${accent.from}44,${accent.to}22,${accent.from}11) 1`,
          borderRadius: 20, padding:"28px 24px", minHeight:400,
          backdropFilter:"blur(16px)",
          boxShadow:`0 0 40px ${accent.from}11, 0 8px 40px #00000099, inset 0 0 40px rgba(0,230,118,0.02)`,
